module stellaris::market_factory {
    ...
    use stellaris::market::{create_pool, calc_market_address};
    ...
    // 错误码常量
    const ERR_EXPIRY_TOO_CLOSE: u64 = 1;
    const ERR_FEE_RATE_TOO_HIGH: u64 = 2;
    const ERR_INITIAL_ANCHOR_TOO_LOW: u64 = 3;
    const ERR_SCALAR_ROOT_ZERO: u64 = 4;
    const ERR_MARKET_ALREADY_EXISTS: u64 = 5;
    ...
    fun create_new_market_internal(
        ...
        assert!(py_state_expiry > utils::now_milliseconds() + market_global::get_min_expiry_interval(), error::out_of_range(ERR_EXPIRY_TOO_CLOSE));
        ...
        assert!(fixed_point64_with_sign::less_or_equal(ln_fee_rate_root_fp, max_ln_fee_rate_root), error::aborted(ERR_FEE_RATE_TOO_HIGH));
        // 检查 initial_anchor 是否 >= 1
        assert!(fixed_point64_with_sign::greater_or_equal(initial_anchor, fixed_point64_with_sign::one()), error::aborted(ERR_INITIAL_ANCHOR_TOO_LOW));
        // 检查 scalar_root 是否不为零
        assert!(!fixed_point64_with_sign::is_zero(scalar_root), error::aborted(ERR_SCALAR_ROOT_ZERO));
        // 检查该市场是否已存在于全局配置中
        let market_pool_address = calc_market_address(
            object::object_address(&py_state_object),
            scalar_root,
            initial_anchor,
            ln_fee_rate_root
        );
        assert!(!market_global::contains(market_pool_address), error::already_exists(ERR_MARKET_ALREADY_EXISTS));
