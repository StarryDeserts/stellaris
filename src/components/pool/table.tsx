...
export default function DefiTable() {
  ...
  const handleRowClick = (crypto: PoolExtended, event: React.MouseEvent) => {
    ...
    router.push(`/Pools/${crypto.poolId}?${queryParams.toString()}`)
  }
    ...
    <main className="p-6">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-100 border-b border-gray-200">
                  ...
                  <th className="text-center py-5 px-6 text-gray-800 font-semibold text-base">
                    Action
                  </th>
                </tr>
              </thead>
      ...
      hover:bg-blue-50 ${index % 2 === 0 ? "bg-white" : "bg-gray-50"}`}
                  ...
                  >
                    ...
                    <td className="py-5 px-5 text-base">
                      <div className="inline-flex items-center justify-center min-w-[80px] w-20 h-8 bg-purple-50 text-purple-700 px-3 py-1 rounded-lg text-sm font-bold border border-purple-200 shadow-sm">
                        {(crypto.baseAPY + 12).toFixed(2)}%
                      </div>
                    </td>
                    <td className="py-5 px-5 text-base text-center">
                      <button 
                        className="px-4 py-2 bg-gradient-to-r from-blue-300/80 to-teal-300/80 hover:from-blue-400/90 hover:to-teal-400/90 text-white rounded-lg transition-all duration-300 font-medium text-sm shadow-sm hover:shadow-md transform hover:-translate-y-0.5"
                        onClick={(e) => {
                          e.stopPropagation();
                          handleRowClick(crypto, e);
                        }}
                      >
                        Add Liquidity
                      </button>
